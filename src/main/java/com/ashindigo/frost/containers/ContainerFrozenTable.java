...
public class ContainerFrozenTable extends Container {
	...
	public void onCraftMatrixChanged(IInventory inventoryIn) {
		for (int i = 0; FrozenTableRecipeAPI.getImmutableApiRecipeList().size() > i; i++) {
			if (FrozenTableRecipeAPI.getImmutableApiRecipeList().get(i).matches(inv, null)) {
				this.getSlot(0).putStack(FrozenTableRecipeAPI.getImmutableApiRecipeList().get(i).getCraftingResult(inv));
				return;
			}
		}
