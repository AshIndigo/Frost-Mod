...
public class FrostEventHandler {
	...
	public void registerBlocks(RegistryEvent.Register<Block> event) {
	    event.getRegistry().registerAll((Block[]) UtilsBlock.modBlocks.get(FrostConstants.MODID).toArray(new Block[]{}));
	    event.getRegistry().register(FrostBlocks.frozenTable);
	}
	
	@SubscribeEvent
	public void registerItems(RegistryEvent.Register<Item> event) {
	    event.getRegistry().registerAll((Item[]) UtilsItem.modItems.get(FrostConstants.MODID).toArray(new Item[]{}));
	    event.getRegistry().register(new ItemBlock(FrostBlocks.frozenTable).setRegistryName(FrostConstants.MODID, "frozentable"));
	    event.getRegistry().register(new ItemBlock(FrostBlocks.frozenOreFragmenter).setRegistryName(FrostConstants.MODID, "frozenorefragmenter"));
	    event.getRegistry().register(new ItemBlock(FrostBlocks.iceDischarger).setRegistryName(FrostConstants.MODID, "icedischarger"));
	    event.getRegistry().register(new ItemBlock(FrostBlocks.iceFreezer).setRegistryName(FrostConstants.MODID, "icefreezer"));
		...
		if (hurtEvent.getEntityLiving() instanceof EntityPlayer) {
			EntityPlayer ep = (EntityPlayer) hurtEvent.getEntityLiving();
				if (hurtEvent.getSource() == DamageSource.DROWN) {
					if (BiomeDictionary.getTypes(UtilsPlayerHelper.getPlayerBiome(ep)).contains(Type.COLD)) {
